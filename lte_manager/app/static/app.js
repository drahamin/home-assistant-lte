...
$('#epc-key-form').onsubmit=async event=>{event.preventDefault();try{const response=await fetch(api('api/epc-routing/key'),{method:'POST',body:new FormData(event.currentTarget)}),data=await response.json();if(!response.ok)throw new Error(data.error);toast('EPC SSH key stored privately');event.currentTarget.reset();loadRoutingAssistant()}catch(e){toast(e.message)}};
