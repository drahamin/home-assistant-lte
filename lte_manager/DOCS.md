...
Set `sim_programming_enabled` to permit USB card access. `sim_reader_name` optionally selects a reader by a case-insensitive name fragment; otherwise `sim_reader_index` selects the detected PC/SC reader. `sim_reader_protocol` can remain `auto` or force `T0`/`T1` for a card that requires it. A physically inserted Gialer programmable LTE USIM is recognized by its exact ATR and receives the `gialersim` type automatically. Keep its ADM in the private `sim_adm_key` Home Assistant password setting, or enter ADM1 only for the current guarded write; credentials are never published or returned by the app. `sim_card_type` may contain another exact pySim model name, or remain blank for matched-profile/automatic detection.

Set `sim_service_provider_name` to the 1–16 character friendly name written to supported cards; its default is `rNET`. The SIM page groups that EF.SPN value with the home PLMN, APN, and network-selection policy as the private carrier profile.

**Program SIM + EPC** requires a complete profile and the exact `PROGRAM <IMSI>` confirmation. It writes ICCID, IMSI, K, OPc, MCC, MNC, MNC length, access class, and the configured EF.SPN friendly name using argument-safe pySim execution. It then reads ICCID, IMSI, and EF.SPN back when available. Only a matching identity read-back proceeds to EPC/HSS provisioning with the same IMSI, K, OPc, AMF, APN, QoS, and AMBR policy; a readable but mismatched EF.SPN also blocks provisioning. If identity read-back fails, the card is not marked deployable and the EPC record is not created. Review EF.PLMNwAcT/EF.OPLMNwAcT, EF.HPLMNwAcT, EF.FPLMN, EF.EPSLOCI, and EF.LOCI because support for these policy files varies by card model.

For a replacement card, choose the existing device under **Profile source**. The app loads only its non-secret device details into the page and uses the saved K/OPc on the server during the confirmed write. Read the target card first to establish its ICCID, and keep the original card offline so two cards do not use the same IMSI. The existing EPC/HSS identity is replaced only after the new card's ICCID and IMSI read back correctly.

The workbench records live validate, write, read-back, and EPC/HSS stages and prevents concurrent writes. If a card is successfully written and its ICCID/IMSI verify but EPC/HSS provisioning fails, the private write profile remains locally in the app as a recovery transaction. Leave or reinsert that card, select **Read inserted SIM**, and use **Recover EPC / HSS**. Recovery requires the exact `RECOVER <IMSI>` confirmation and a second matching card-identity read before provisioning. A completed recovery removes the temporary transaction, and abandoned transactions expire after 30 days. K, OPc, and ADM are never returned by the recovery API or added to activity logs.
